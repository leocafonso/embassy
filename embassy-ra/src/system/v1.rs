//! Clock configuration for RA2 family (RA2E1, RA2E2, RA2E3, RA2L1, …) — sysc_v1
...
pub(crate) fn init(config: Config) -> Clocks {
    ...
    let source_freq = match config.source {
        ClockSource::Hoco => {
             // For RA2, HOCO frequency is set in Option Bytes (OFS1 at 0x0000_0404).
             // 0x0000_0408 is SECMPU, NOT OFS1 — reading from there always yields
             // 0xFFFFFFFF which falls through to the default 48 MHz.
             // Bits [14:12] of OFS1 determine frequency:
